...
static void fill_addr4(net_card*card,const struct net_device*dev)
{
	...
	if(!ifa)
		return;
	__builtin_memcpy(card->addr4.ip.bytes,&ifa->ifa_local,sizeof(card->addr4.ip.bytes));
...
static void fill_addr6(net_card*card,const struct net_device*dev)
{
	...
	list_for_each_entry(ifa,&idev->addr_list,if_list){
		...
		if(ipv6_addr_type(&ifa->addr)!=IPV6_ADDR_UNICAST||
			...
			continue;
		__builtin_memcpy(card->addr6.ip.bytes,ifa->addr.s6_addr,sizeof(card->addr6.ip.bytes));
...
static void fill_route4(net_card*card,const struct net_device*dev)
{
	...
	if(rt->rt_uses_gateway&&rt->rt_gw_family==AF_INET){
		__builtin_memcpy(card->gateway4.bytes,&rt->rt_gw4,sizeof(card->gateway4.bytes));
		card->gw4=1;
	}
	ip_rt_put(rt);
}
static void fill_route6(net_card*card,const struct net_device*dev)
{
	...
	__builtin_memcpy(fl6.saddr.s6_addr,card->addr6.ip.bytes,sizeof(card->addr6.ip.bytes));
	...
	if(res.nh&&res.nh->fib_nh_dev==dev&&res.nh->fib_nh_gw_family==AF_INET6){
		__builtin_memcpy(card->gateway6.bytes,res.nh->fib_nh_gw6.s6_addr,sizeof(card->gateway6.bytes));
		card->gw6=1;
	}
	rcu_read_unlock();
}
static void fill_card(net_card*card,const struct net_device*dev)
{
	...
	if(dev->addr_len==sizeof(card->mac)){
		__builtin_memcpy(card->mac,dev->dev_addr,sizeof(card->mac));
		card->has_mac=1;
	}
