...
{
	return addr->prefix > 32 ? ERR_INVAL : STATUS_OK;
}
static status net_addr6_valid(const net_addr6 *addr)
{
	return addr->prefix > 128 ? ERR_INVAL : STATUS_OK;
}
...
status net_card_validate(const net_card *card)
{
	...
	if (!card->present)
		return ERR_STATE;
	status ret;
	if ((ret = net_text_valid(card->name, sizeof card->name, 1)) ||
		(ret = net_text_valid(card->driver, sizeof card->driver, 0)) ||
		(ret = net_text_valid(card->bus, sizeof card->bus, 0)))
		return ret;
...
status net_plan_validate(const net_plan *plan)
{
	if (!plan->card_id)
		return ERR_INVAL;
	if (net_text_valid(plan->name, sizeof plan->name, 1))
		return ERR_INVAL;
...
static void fill_addr4(net_card *card, const struct net_device *dev)
{
	...
	for (u8 i = 0; i < sizeof card->addr4.ip.bytes; i++)
...
static void fill_addr6(net_card *card, const struct net_device *dev)
{
	...
	list_for_each_entry(ifa, &idev->addr_list, if_list) {
		...
		if (ipv6_addr_type(&ifa->addr) != IPV6_ADDR_UNICAST ||
			...
			continue;
		for (u8 i = 0; i < sizeof card->addr6.ip.bytes; i++)
...
static void fill_card(net_card *card, const struct net_device *dev)
{
	...
	if (dev->addr_len == sizeof card->mac) {
		for (u8 i = 0; i < sizeof card->mac; i++)
